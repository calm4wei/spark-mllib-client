...
object Test {
    ...
    def main(args: Array[String]) {
        ...
        //        val result = MatrixService.multiply(x, y)
        //        println(result)

        //        val loggedData = new LoggedDataAccess
        //        loggedData.query("find math data!")

        //        val x = new FileLineTraversable(new File("D:\\data\\iris\\iris.data"))
        //        for {line <- x.take(2)} yield println(line)

        //        val tree = Branch(1, Leaf(2), Branch(3, Leaf(4), NilTree))
        //        BinaryTree.traverse(tree)(println)

        val l1 = List("a", "b", "c")
        val l2 = List(1, 2, 3)
        println(l1 zip l2)
        println(l1 zip (Stream from 2))
    }


    def createConnection(conn_url: Option[String],
                         conn_user: Option[String],
                         conn_pw: Option[String]): Option[Connection] = for {
        url <- conn_url
        user <- conn_user
        pw <- conn_pw
